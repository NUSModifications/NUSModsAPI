...
describe('getCacheFilePath', () => {
  ...
  it('should normalize url', () => {
    ...
    expect(getFilePath('http://www.example.com/', 'a=1&b=2')).toBe(getFilePath('http://www.example.com/', 'b=2&a=1'));
    expect(getFilePath('http://www.example.com/?a=1', 'b=2')).toBe(getFilePath('http://www.example.com/?b=2', 'a=1'));
  });
  ...
  it('should split subpaths to many different subfolders', () => {
    ...
    expect(getFilePath('https://www.example.com/1/2/', 'query=123&x=5')).toBe('testBase/example.com/1/2/query=123&x=5');
...
describe('getFileModifiedTime', () => {
  ...
  it('should output infinity if it is not a file', async () => {
    expect(await getFileModifiedTime('testFolder')).toBe(Infinity);
  });

  it('should output infinity if no file', async () => {
    expect(await getFileModifiedTime('testNoFile')).toBe(Infinity);
  });
});
